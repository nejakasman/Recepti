...
async function updatePorcije(receptId, porcije) {
  try {
    ...
    const response = await fetch(
      `http://localhost:8080/api/recepti/${receptId}/sestavine?porcije=${porcije}`
    );
...
async function viewReceptDetails(receptId) {
  try {
    ...
    localStorage.setItem("podrobni_recept", receptId);
    ...
    receptDetailsDiv.innerHTML = `
            ...
            <input type="number" id="porcije" value="${porcije}" min="1" onchange="updatePorcije(${
      recept.id
    }, this.value)" />
...
function normalizeSestavina(sestavina) {
  return sestavina
    .replace(/^\d+\s*(g|kg|ml|l|oz|tbsp|tsp|cup)?\s*/i, "")
    .trim()
    .toLowerCase();
}
async function getPogostostSestavin() {
  try {
    const response = await fetch(
      "http://localhost:8080/api/recepti/pogostost-sestavin"
    );
    const pogostost = await response.json();
    console.log("Pogostost sestavin:", pogostost);

    const receptResponse = await fetch(
      "http://localhost:8080/api/recepti/recepti"
    );
    ...
    if (Object.keys(pogostost).length === 0) {
      ...
      const sestavineList = Object.keys(pogostost).sort(
        (a, b) => pogostost[b] - pogostost[a]
      );

      // Funkcija za sortiranje receptov glede na sestavine
      function sortReceptiPoSestavinah(recepti, sestavina) {
        recepti.forEach((recept) => {
          const vsebujeSestavino = recept.sestavine.some(
            (s) => normalizeSestavina(s) === sestavina
          );
      ...
      for (let i = 0; i < sestavineList.length; i++) {
        ...
        razvrščeniRecepti = [
          ...razvrščeniRecepti,
          ...recepti.filter((r) => r.vsebujeSestavino),
        ];
...
async function zabeleziOgled(receptId) {
  try {
    ...
    const response = await fetch(
      `http://localhost:8080/api/recepti/${receptId}/ogled`,
      {
        method: "POST",
      }
    );
    ...
    if (!response.ok) {
      ...
      let ogledaniRecepti =
        JSON.parse(localStorage.getItem("ogledaniRecepti")) || [];
      if (!ogledaniRecepti.includes(receptId)) {
        ogledaniRecepti.push(receptId);
        localStorage.setItem(
          "ogledaniRecepti",
          JSON.stringify(ogledaniRecepti)
        );
      }
    }
  } catch (error) {
